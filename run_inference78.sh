...
#PBS -q eleceng
#PBS -N LSeq_Inference_78

# ===== EDIT THIS BEFORE EACH RUN =====
RUN_NUMBER=1

# ===== PATHS =====
BASE_DIR="/srv/scratch/z5423210/StanleyThesis2026/lseqsleep/L-SeqSleepNet/sleepedf-78"
SCRIPT_DIR="$BASE_DIR/network/lseqsleepnet"
OUT_BASE="/srv/scratch/z5423210/StanleyThesis2026/prev_runs/sleepedf-78/run${RUN_NUMBER}"
...
echo "Running inference for sleepedf-78 run${RUN_NUMBER}..."

for i in {1..10}
do
  FINETUNED="/srv/scratch/z5423210/StanleyThesis2026/prev_runs/sleepedf-78/run${RUN_NUMBER}/n${i}/checkpoint"
  ...
  apptainer exec --nv -B /srv:/srv $CONTAINER \
    bash -c "
      ...
      python test_lseqsleepnet.py \
        ...
        --eeg_test_data '../../file_list_30min/eeg/test_list_n${i}.txt' \
        --out_dir '$OUT_DIR/' \
...
echo "All inference complete for sleepedf-78 run${RUN_NUMBER}."
