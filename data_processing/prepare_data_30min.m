...
raw_data_path = '/srv/scratch/z5423210/StanleyThesis2026/lseqsleep/L-SeqSleepNet/sleepedf-78/raw_data_30min/';
mat_path = './mat_30min/';
...
for i = 1 : numel(list)
    ...
    out_file_eeg = [mat_path, 'n', num2str(count,'%02d'), '_', num2str(night), '_eeg.mat'];
    out_file_eog = [mat_path, 'n', num2str(count,'%02d'), '_', num2str(night), '_eog.mat'];
    if exist(out_file_eeg, 'file') && exist(out_file_eog, 'file')
        disp(['Skipping (already exists): n', num2str(count,'%02d'), '_', num2str(night)]);
        continue;
    end
