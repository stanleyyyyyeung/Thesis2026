...
data_path = '/srv/scratch/z5423210/StanleyThesis2026/lseqsleep/L-SeqSleepNet/sleepedf-78/edfFiles/';
...
for i = 1 : numel(listing)
    ...
    out_file = [raw_data_path, 'n', num2str(sub_id,'%02d'), '_', num2str(night), '.mat'];
    if exist(out_file, 'file')
        disp(['Skipping (already exists): n', num2str(sub_id,'%02d'), '_', num2str(night)]);
        continue;
    end
    ...
    chan_ind_eeg = find(ismember(channel_names, 'EEGFpzCz') | ismember(channel_names, 'EEG Fpz-Cz'));
    if(isempty(chan_ind_eeg))
        disp(['Oops, EEG channel not found for ', listing(i).name, ', skipping.']);
        continue;
    end
    
    if(header.frequency(chan_ind_eeg) ~= fs)
        disp(['Oops, EEG sampling frequency mismatch for ', listing(i).name, ', skipping.']);
        continue;
    end
    
    chan_ind_eog = find(ismember(channel_names, 'EOGhorizontal') | ismember(channel_names, 'EOG horizontal'));
    if(isempty(chan_ind_eog))
        disp(['Oops, EOG channel not found for ', listing(i).name, ', skipping.']);
        continue;
    end
    
    if(header.frequency(chan_ind_eog) ~= fs)
        disp(['Oops, EOG sampling frequency mismatch for ', listing(i).name, ', skipping.']);
        continue;
    end
    ...
    %% Preprocessing Filter coefficients
    Nfir = 100;
    ...
    save(out_file, 'data', 'label', 'y', '-v7.3');
    clear data label y
end
