function [chan_data_eeg_new, chan_data_eog_new, hypnogram_new] = edfx_process_time_2chan_moreW(target_dir, chan_data_eeg, chan_data_eog, hypnogram, epoch_time, fs)
    ...
    end_eeg = min(end_eeg, length(chan_data_eeg));
    epochs_to_use = floor((end_eeg - start_eeg + 1) / (epoch_time * fs));
    end_eeg = start_eeg + epochs_to_use * epoch_time * fs - 1;
