...
class LoadDataset(object):
    ...
    def load_path(self):
        ...
        for subject_num, (subject_seq, subject_label) in enumerate(zip(subject_dirs_seq, subject_dirs_labels), start=1):
            subject_pairs = []
            if os.path.isdir(subject_seq) and os.path.isdir(subject_label):
                ...
                print(f"WARNING: subject {subject_num} directory missing "
                      f"({subject_seq}) — skipping (0 samples contributed). "
                      f"Expected for excluded subject 8; unexpected for any other subject.")
