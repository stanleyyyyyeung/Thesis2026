...
# keep resident at once, PER DATALOADER WORKER (each worker process gets
# its own independent cache — they don't share). Filtering a recording is
# the expensive step; caching means a study's later windows (there are
# usually several per recording) don't re-pay that cost. Higher = fewer
# repeated filter passes but more RAM; each cached recording is roughly
# (n_channels * n_samples * 8 bytes) since MNE stores float64 internally —
# for an ~8hr recording at 200Hz that's about 6 * 8*3600*200 * 8 bytes
# =~ 280MB per cached study. Tune against (available RAM per worker) /
# 280MB, leaving headroom for everything else in that worker process.
CACHE_SIZE = 4
