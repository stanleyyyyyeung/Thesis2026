...
tf.app.flags.DEFINE_string("pretrained_model", " ", "Path to pretrained model checkpoint") # Specifies the path to pretrained weights
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        vars_in_checkpoint = tf.train.list_variables(FLAGS.pretrained_model)
        vars_in_checkpoint_names = [v[0] for v in vars_in_checkpoint]
        var_list_to_restore = [v for v in variables if v.name[:-2] in vars_in_checkpoint_names]
        restorer = tf.train.Saver(var_list_to_restore)
